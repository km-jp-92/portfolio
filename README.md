...
- 発表者が資料をページ送りすると、その資料を閲覧しているユーザー全員がリアルタイム同期する機能
...
- PDFのURL（Cloudinary）への直接アクセス防止
- 資料は1週間で自動削除（Rakeタスク、Cloudinary API）
...
5. 該当ユーザーが参加者に「同期ありURL」とパスワードを共有。参加者が「同期ありURL」にアクセスすると、サーバーが資料アップロードURLにアップロード済みのPDFを参照して表示。「発表者になる」ボタンを押すと、発表者のPDFページと参加者に見えるPDFページが同期される

■ PDFのリアルタイム同期の実装について  
以下の方針で実装予定です。  
- 「発表者になる」ボタンを押す
- フロント側で、ボタンが押されたかどうかを使ってフラグを切り替える。押されていれば自分のページ位置がActionCableを通じて全ブラウザに送信される。押されていなければ追随する。
...
■ ER図  
  [ER図（Gyazoへのリンク）](https://gyazo.com/ac00de2660e24243c1bff25a31b63572)
